...
execute if score @p players matches ..0 run tellraw @a [{"text":"There are currently no players on the "},{"bold": true,"color":"red","text":"Players"},{"bold": false,"color":"white","text":" team. Please join the team, if you want to play"}]
execute if score @p players matches ..0 run scoreboard players set $start_not_possible sign 1

execute if score @p players matches 9.. run tellraw @a [{"text":"There is currently "},{"score": {"name":"@p","objective":"players"}},{"text":"players on the "},{"bold": true,"color":"red","text":"Players"},{"bold": false,"color":"white","text":" team. Some of the will have to join the spectator team if you want to play, as the player limit is 8 players"}]
